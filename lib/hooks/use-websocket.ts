...
export const useWebSocket = () => {
  ...
  const joinRoom = (eventId: number) => {
    if (!user) throw new Error('User not authenticated');
    joinEventRoom(eventId, user.id);
  };

  const lockEventTickets = async (eventId: number, categoryId: number, quantity: number) => {
    if (!user) throw new Error('User not authenticated');
    return await lockTickets(eventId, categoryId, quantity, user.id);
  };

  const unlockEventTickets = async (eventId: number, categoryId: number, quantity: number) => {
    if (!user) throw new Error('User not authenticated');
